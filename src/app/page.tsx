...
function LoadingScreen() {
  return (
    <div className="w-screen h-screen flex flex-col items-center justify-center bg-[var(--bg-void)]">
      <div className="w-8 h-8 border-2 border-white/[0.08] border-t-[var(--cyan)] rounded-full animate-spin" />
      <p className="text-[10px] font-mono uppercase tracking-[0.25em] text-[var(--text-dim)] mt-5">Initializing System</p>
    </div>
  );
}
      ...
      <div className="flex-1 min-h-0 flex">
        {user && screen ? (
          ...
          <div className="flex-1 flex items-center justify-center bg-mesh">
            <div className="glass rounded-2xl px-12 py-8 text-center animate-fade-up">
              ...
              <p className="text-[10px] font-mono text-[var(--text-dim)] mt-1">Enter credentials in the top bar</p>
            </div>
          </div>
        )}
      </div>

...
function ScreenContent({ screen }: { screen: ScreenKey }) {
  ...
  if (screen.includes("BERTH")) return <BerthMonitor key={screen} terminal={terminal} />;
  if (screen.includes("GATE")) return <GateMonitor key={screen} terminal={terminal} />;
...
function Dashboard() {
  ...
  const [screen, setScreen] = useState<ScreenKey | null>(() => {
    if (!user) return null;
    const enabled = (Object.entries(user.screens) as [ScreenKey, boolean][])
      .filter(([, v]) => v)
      .map(([k]) => k);
    return sortScreens(enabled)[0] ?? null;
  });

...
function getBlockTypeColor(blockType: string | null | undefined): string {
  if (!blockType) return "#64748b";
  const b = blockType.toUpperCase();
  if (b === "DG") return "#dc2626";
  if (b === "RF") return "#06b6d4";
  if (b === "EMPTY") return "#64748b";
...
function getStatusIndicator(card: EquCard) {
  if (card.isOnline) {
    return (
      <span className="relative flex h-3 w-3 shrink-0">
        <span className="animate-pulse absolute inline-flex h-full w-full rounded-full bg-[#10b981] opacity-75" />
  ...
  return (
    <span className="text-[8px] font-mono font-black uppercase px-1.5 py-0.5 rounded bg-[#64748b]/20 text-[#64748b]">
      Offline
    </span>
  );
}
...
function getTTTBadgeStyle(minutes: number | null | undefined): { bg: string; text: string; shadow: string; border: string } {
  if (minutes == null) return { bg: "rgba(0,0,0,0.3)", text: "#64748b", shadow: "", border: "" };
  ...
  return { bg: "rgba(0,0,0,0.3)", text: "#94a3b8", shadow: "", border: "" };
}

function getMPHColor(mph: number): string {
  if (mph >= 25) return "#10b981";
  if (mph >= 15) return "#00f0ff";
  if (mph > 0) return "#f59e0b";
  return "#64748b";
}

function EquCardRich({ card }: { card: EquCard }) {
  const accent = EQU_ACCENTS[card.equType] ?? "#64748b";
  const posColor = getBlockTypeColor(card.position);
  const tttBadge = getTTTBadgeStyle(card.tttMinutes);

  return (
    <div className="relative flex flex-col bg-[#0e1321] border border-[#1c273e] rounded-lg overflow-hidden shadow-lg hover:border-[#00f0ff]/30 transition-all">
      {/* Gradient background */}
      ...
      <div className="relative p-2.5 flex flex-col gap-1.5">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="min-w-0">
              <div className="font-mono font-black text-sm text-[#dee2f6] uppercase tracking-wide truncate">
                {card.displayName}
              </div>
              <div className="text-[10px] font-mono text-[#94a3b8] truncate uppercase">
                {card.driverName ?? "NO DRIVER"}
                {!card.driverName && (
                  <svg className="w-3 h-3 inline ml-1 text-[#64748b]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        {/* MPH large display */}
        <div className="flex items-center justify-between">
          <div className="bg-[#060a14] px-2 py-0.5 rounded shadow-inner border border-[#1c273e]">
            ...
            <span className="text-[8px] font-mono font-bold uppercase text-[#64748b] ml-1">MPH</span>
          </div>
        </div>
      </div>
    </div>
...
function YTCardCompact({ card }: { card: EquCard }) {
  ...
  return (
    <div className="flex flex-col bg-[#0e1321] border border-[#1c273e] rounded p-2 gap-1 min-w-[120px]">
      <div className="flex items-center justify-between">
        <span className="flex items-center gap-1.5 min-w-0">
          <EquipmentIcon equType={card.equType} className="w-7 h-7 shrink-0" style={{ color: accent }} />
          <span className="font-mono font-bold text-[10px] text-[#dee2f6] truncate">{card.displayName}</span>
        </span>
        {getStatusIndicator(card)}
      </div>
      <div className="text-[9px] font-mono text-[#94a3b8] truncate">{card.driverName ?? "NO DRIVER"}</div>
      ...
      <div className="flex items-center justify-between">
        <span className="text-[9px] font-mono tabular-nums" style={{ color: getMPHColor(card.movesLastHour) }}>
          {card.movesLastHour} <span className="text-[#64748b]">MPH</span>
        </span>
      </div>
    </div>
  );
...
function QCGroupCard({ group, compact }: { group: EquipmentData["qcGroups"][number]; compact: boolean }) {
  ...
  const accentColor = isTypeA ? "#f59e0b" : "#00f0ff";
  const headerBg = "#141c2e";
  ...
  return (
    <div className="w-full flex items-stretch rounded-xl border-2 shadow-md overflow-hidden" style={{ background: gradient, borderColor: `${accentColor}44`, minHeight: "var(--qc-row-height, 80px)" }}>
      ...
      {group.pendingOrdersCount > 0 && (
        <div className="flex items-center justify-center px-3">
          <div className="bg-[#f59e0b]/20 px-3 py-1 rounded-md border border-[#f59e0b]/30 text-center">
...
function YardSectionCard({ cards, label, equType, accentColor }: { cards: EquCard[]; label: string; equType: EquType; accentColor?: string }) {
  const accent = accentColor ?? EQU_ACCENTS[equType] ?? "#64748b";
  return (
    <div className="flex flex-col gap-1">
      <div className="flex items-center gap-2 px-0.5 shrink-0">
        ...
        <span className="text-[11px] font-mono font-black uppercase tracking-[0.2em] text-[#94a3b8]">{label}</span>
        <span className="text-[8px] font-mono font-bold px-1.5 py-0.5 rounded-full bg-[#0e1321] border border-[#1c273e] text-[#64748b]">{cards.length}</span>
    ...
    return (
      <>
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-[#64748b]">
          ...
          <p className="text-xs font-mono uppercase tracking-[0.2em]">Connecting to Equipment Database</p>
        </div>
      </>
    );
  }
      ...
      <>
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="border border-[#ef4444]/50 bg-[#ef4444]/10 px-8 py-6 text-center max-w-md rounded-xl">
            ...
            <button className="px-4 py-1.5 text-[11px] font-mono font-bold uppercase tracking-wider text-white bg-[#ef4444] hover:bg-[#ef4444]/90 rounded">Retry</button>
          </div>
        </div>
      </>
    );
    ...
    <>
      ...
      <div className="flex-1 flex items-center justify-center">
        <div className="border border-[#1c273e] px-12 py-8 text-center rounded-xl">
          <div className="text-xs font-bold font-mono uppercase tracking-widest text-[#64748b] mb-2">No Active Equipment</div>
          <p className="text-[11px] font-mono text-[#64748b]">No equipment data available at this time.</p>
        </div>
      </div>
    </>
  );
      ...
      <main className="flex-1 min-h-0 overflow-hidden p-2 flex flex-col gap-2">
        ...
        <div className="flex flex-col gap-[clamp(4px,0.7vh,10px)] min-h-0 flex-1">
          <div className="flex items-center gap-2 px-1 shrink-0">
            <span className="text-xs font-mono font-black uppercase tracking-[0.3em] text-[#94a3b8]">Vessel Operations</span>
            <div className="h-px flex-1 bg-[#1c273e] opacity-30" />
          </div>
        ...
        <div className="flex-1 min-h-0 flex flex-col">
          <div className="flex items-center gap-2 px-1 mt-2 shrink-0">
            <span className="text-xs font-mono font-black uppercase tracking-[0.3em] text-[#94a3b8]">Yard Operations</span>
            <div className="h-px flex-1 bg-[#1c273e] opacity-30" />
          </div>
